...
<AppShell>
  <header
    class="border-b border-[var(--color-border-subtle)] bg-[var(--color-bg)] px-3 py-3 sm:px-5 sm:py-4"
  >
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0">
        <div class="mb-1 text-xs text-[var(--color-text-muted)]">
          fizza / projects
        </div>
  ...
  <main class="min-h-0 flex-1 overflow-hidden">
    {#if $projectsQuery.isPending}
      <div class="p-8 text-sm text-[var(--color-text-muted)]">Loading…</div>
    ...
    {:else if !$projectsQuery.data?.length}
      ...
      />
    {:else}
      <div class="h-full overflow-y-auto px-3 py-4 sm:px-5 sm:py-5">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {#each $projectsQuery.data as p (p.id)}
            {@const active = hint?.project === p.name}
            <div
              class={
                "group relative rounded-3xl border p-4 text-left transition sm:p-5 " +
                (active
              ...
              <div class="relative z-10 pointer-events-none">
                <div class="mb-2 flex items-start justify-between gap-2">
                  ...
                  <div class="flex shrink-0 items-center gap-1">
                    {#if active}
                      <span
                        class="rounded-lg bg-[var(--color-accent)]/15 px-2 py-0.5 text-[10px] font-medium text-[var(--color-accent)]"
                      >
                        Recent
                      </span>
                    {/if}
                    <button
                      ...
                      class="pointer-events-auto cursor-pointer rounded-lg px-1.5 py-0.5 text-xs text-[var(--color-text-muted)] opacity-100 transition hover:bg-[var(--color-danger)]/10 hover:text-[var(--color-danger)] sm:opacity-0 sm:group-hover:opacity-100"
                ...
                <p
                  class="line-clamp-2 min-h-10 text-sm text-[var(--color-text-muted)]"
                >
